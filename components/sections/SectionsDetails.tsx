  ...
  return (
    <div className="px-6 py-4 flex flex-col gap-5">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center">
        <h1 className="text-2xl font-bold mb-4 md:mb-0">{section.title}</h1>

        <div className="flex gap-4">
          <SectionMenu course={course} />
          {!purchase ? (
            <Button onClick={buyCourse} disabled={isLoading}>
              {isLoading ? (
      ...
      {isLocked ? (
        <div className="px-10 flex flex-col gap-5 items-center bg-[#FFF8EB] rounded-lg p-4">
          <Lock className="h-8 w-8" />
          <p className="text-sm font-bold text-center">
            This Section video is locked. Please purchase the course to gain
            access.
          </p>
        </div>
      ) : (
        ...
        {resources.length > 0 ? (
          resources.map((resource) => (
            <Link
              ...
              target="_blank"
              className="flex items-center bg-[#FFF8EB] rounded-lg text-sm font-medium p-3 mb-2"
            >
